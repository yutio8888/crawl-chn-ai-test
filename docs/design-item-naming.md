...
> **历史设计记录，基线为上游 0.34.1。** 本文保留物品命名审查时的分析和候选
> 方案，不承担当前待办跟踪。现行命名裁定以
> [`docs/decisions.md`](decisions.md) 的 active 条目为准；仍需行动的问题以
...
审查时的系统已实现大部分物品翻译覆盖，但仍发现以下命名一致性问题。
...
`D-B-013` 最终决定珠宝保持 `effect戒指/项链`，不统一添加“之”；下面三项是
当时评估过但没有执行的候选步骤：
...
这些观察不是当前 backlog。若在当前默认分支仍可复现，应以当前代码和数据重新取证，
再创建或更新 GitHub Issue。

1. **部分翻译可能需重新审校**：当时 `source.txt` 中的 weapon_ego 条目（如 DRag→雷击、repulsion→排斥等）是否与游戏内效果描述一致，需逐个验证。
