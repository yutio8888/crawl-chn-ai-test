...
- P2 长背包已用 41 件物品补测：详情取消返回保持滚动位置
  ...
  再打开背包从顶部开始（`longpack-reopened`）。用户明确确认不要求
  跨关闭重开保持位置，§3.4 已同步，验收缺口闭合。两种不可用原因均已通过，
...
- P2/P3 均已推送并完成两域内联审核；等待适用 GitHub Actions 通过后顺序合并。

代码及设备验收的阻塞项已闭合；最终 Ready 与合并记录绑定 PR #131 / #132 的
实际候选及适用 CI 结果，不以本地报告代替 GitHub Actions。

## 最终审核边界

P2 代码候选 `b915769e5c`，后续源码仅修正注释空格；P3 功能代码候选 `aa9bd4ad43`，
后续更新验收文档、注释空格及下述扫描器回归测试，复用功能内容及依赖未变化的验证。
`classify_reviewers.py` 对 P2 相对 `8745b0af18`、P3 相对 P2 的差异均为 mixed，
两域已内联检查。P1 原未跟踪设计稿已通过命名 stash 保留，未覆盖或删除。

已发现的实现阻塞及背包验收口径均闭合。用户已明确授权公开推送、审核与合并；
PR #131 / #132 的最新候选 CI 及最终合并记录为交付依据。
原 P2 CI run `34017568264` 成功，但不覆盖新修复，未用旧结果替代新候选 CI。
...
扫描器修正 `50275515cb`：专项 29 项通过；code profile
`d52eb24af3..50275515cb`，Run ID
`20260906T082358842360042+0000-1915444-50275515cbdc`，Failures 0。
