...
| P3 | 地图“更多”、瞄准高级动作、原生溢出弹窗 | 排除区域、路径点、强制确认等高级操作触屏可达 |
...
- 丢弃：进入多选、分类切换、数量及确认；详情取消返回保留背包滚动位置。
  2026-09-06 用户确认：执行丢弃关闭菜单后，再次打开背包不要求恢复之前的位置。
- 喊叫：全部子命令、盟友目标选择与取消不需完整键盘，回合消耗与原版一致。
- 搜索：`Ctrl-F` 等价流程直接进入，结果页已有 `/` `=` 情境键继续可用。

## 4. 第三阶段：“更多”机制、地图与瞄准高级动作、溢出页面

前置：P2 合并。设计决策记录在本节（`docs/decisions.md` 是翻译裁决登记表，不用于
UI 机制决策）。

### 4.1 设计决策（2026-09-06 已定）

**D6 不扩展 Ctrl 情境桥，改用原生“更多”弹窗。** 原方案要把控制字符经
`sendContextKey()` → `nativeKeyboardKey` 送入 SDL，需要新的输入面并逐页白名单。改为：
`InputActionScope` 附带一个溢出动作列表 `more`；页面在某个槽发布哨兵键
`ui::INPUT_MORE_KEY`（`CK_F10`），Java 显示为“更多”；`UIRoot::deliver_event` 拦截该键，
用抽屉同款原生弹窗列出溢出动作，选中后 `macro_buf_add(key)` 把键排入宏缓冲，由页面
按自身规则处理。控制键因此永远不经过 Java；原生桥白名单只增加 `CK_F10` 本身。

**D7 溢出条目必须有原生标签。** 弹窗由 C++ 绘制，不能像六槽那样由 Java 按
`(screen, slot, key)` 取资源。地图与瞄准的条目用 `get_command_description(cmd, true)`
（命令描述库已有中英文）加 `command_to_key(cmd)`（随键位重绑定）；技能、使用物品、
箭袋菜单的候选改用 `T_()` 原生标签，译文与 Android 资源用词一致。

**D8 溢出规则统一为 `ui::spill_input_actions()`**：从起始槽依次填入候选；放不下时最后
一个直接槽变为“更多”，被挤出的动作排在列表首位，保持可见顺序，不丢任何候选。

### 4.2 接入范围

| 页面 | 直接槽 | “更多”内容 |
|---|---|---|
| 大地图 | 槽 0 = 更多；返回、上下楼梯、传送门、陷阱不变 | 排除区域、半径排除（随后数字提示进入 TEXT 上下文）、清除排除、查找排除、路径点、查找路径点、祭坛、藏品、自身、上一层、下一层、跳转楼层、层注释、描述、探索、帮助 |
| 瞄准 | 槽 5 = 更多；确定、取消、上下目标、自身不变 | 终点确认、强制确认、强制终点确认、箭袋前后切换、选择动作、对象前后循环、切换路径显示、排除、描述、查找楼梯/祭坛/传送门/陷阱、帮助 |
| 物品描述 | 最多四个候选；溢出时前三个加“更多” | 第四个及以后候选（铭刻、调整字母、箭袋、技能目标等） |
| 使用物品 | 最多四个候选；溢出时前三个加“更多” | 第四、第五个候选 |
| 技能 | 最多四个候选；溢出时前三个加“更多” | 第四个及以后候选，含全部技能、帮助等 |
| 箭袋选择 | 不变（聚焦模式 + 前三个候选） | 暂不接入：`quiver.cc` 含两处 `#if` 切分的 if/else 链，tree-sitter 扫描器无法解析，改动该文件会使 code profile 失败；待 issue #120 方案 3 或登记精确模式后再接 |

### 4.3 验收

- 地图：更多 → 排除区域后地图上出现排除标记；半径排除进入数字提示且完整键盘自动展开；
  清除排除生效；路径点建立与查找闭环；弹窗取消后回到 MAP 行。
- 瞄准：强制确认与普通确认分别校验；箭袋切换与对象循环生效；取消后回到 TARGET 行。
- 物品描述：候选多于四个的物品能在“更多”中看到并执行后排动作。
- 技能页：“更多”里能到达全部技能与帮助。
- 嵌套：弹窗打开时第四行不变；关闭后无残留键。
...
| “更多”产生嵌套输入 | 弹窗保留调用页描述符，但不继承其溢出列表；控制键留在原生宏缓冲，Java 桥只增加 CK_F10 |
...
| 6 | P3 设计决策 | 本文 §4.1 记录“更多”机制与输入桥的范围 |
