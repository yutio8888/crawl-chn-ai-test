...
> 发布决策更新（2026-07-23）：发布负责人已冻结首版为 `0.34.1-zh1`，仅包含
> Windows Tiles 与 macOS Tiles。Linux 只保留 CI 编译验证，不生成正式发布资产；
> Android 继续延期。因此下文关于“评估时尚未冻结版本/范围”的内容保留为原始审计记录，
> 不再是当前候选的未决事项。
>
...
- **正式发布：条件性 No-Go。** 版本号和首版平台范围已经冻结；在发布流水线候选合并、
  标签 CI、Windows/macOS 人工验收和草稿批准完成前，仍不得公开正式 Release。
...
首版已经明确限定为 Windows/macOS Tiles，#8、#9 延期且不阻塞首版。Linux 同样不属于
首版正式资产，只保留 CI 编译质量信号；这些延期会写入 release scope 和 known issues。
...
4. 对计划发布的平台做安装后 smoke：
   ...
   - Linux/Android：本版不生成正式资产，后续纳入时另行补齐平台验收。
...
- **当前：先完成 `0.34.1-zh1` 的候选合并、标签 CI 和 Windows/macOS 人工验收。**
- **Windows/macOS 首版：完成上述最小清单后可转 Go；目前没有发现需要大规模返工的代码缺陷。**
- **Linux/Android：本版延期，不阻塞 `0.34.1-zh1`。**
