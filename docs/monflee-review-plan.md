...
- 规范身份来源是生产 C++ SpeakDB artifact 中 effective source 为
  `database/monflee.txt` 的完整 key 集。Python inventory 只消费 production
  artifact，不实现第二套 TextDB parser。
...
该 identity 有五个有序变体：权重依次为 `30, 10, 10, 10, 10`。第 0 项是普通
声音消息；第 1–4 项带 `VISUAL:`，属于仅可见时显示、且不因沉默消失的视觉消息。
运行时 token 依次为 `@The_monster@`、`@The_monster@`、
`@The_monster@`、`@monster@`、`@The_monster@`。
...
Production artifact 的完整 source snapshot 必须与这些文件所在的精确 Git OID
绑定；只比较当前 worktree 或 source token 不足以证明 production 输入。
...
- `mon-behv.cc:1271-1297` 在 `ME_SCARE` 中查询
  `mon->name(DESC_DBNAME) + " flee"`，随后切换逃跑行为。
- `dat/des/altar/xom_sheep.des:42-44` 对可见的 Xom 梦羊直接查询同一稳定 key。
- `mon-speak.cc:851-905` 展开 `@...@` token，剥离 `VISUAL:`，按可见性与沉默
  规则路由至 `MSGCH_TALK` 或 `MSGCH_TALK_VISUAL`。

## 验收标准

1. Production EN/ZH artifact schema、source snapshots、key、provenance、variant
   locator、ordinal、weight 与 raw pattern 均严格验证；parse error、override、重复
   identity、ordinal gap 或未识别控制前缀 fail closed。
...
先由 production C++ 生成 exact-OID EN/ZH artifact，再交给窄域 consumer：
