...
string _shout_key(const monster &mons)
{
    ...
    // Everything else should use the canonical English DB lookup name, not
    // the translated display name, so shout.txt keeps stable English keys.
    return mons_type_name_en(mons.type, DESC_DBNAME);
}
...
void yell(const actor* target)
{
    ...
    else
    {
        const char *fugue_suff = you.duration[DUR_FUGUE] ?
            T_(", and the damned howl along") : "";
        mprf(MSGCH_SOUND, T_("You %s%s%s!"),
             shout_verb.c_str(),
             you.berserk() ? T_(" wildly") : T_(" for attention"),
             fugue_suff);
    }
