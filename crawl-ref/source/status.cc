...
bool fill_status_info(int status, status_info& inf)
{
    ...
    case STATUS_STAT_ZERO:
    {
        ...
        inf.light_text   = C_("status", "Crippled");
        inf.db_key       = "Crippled";
        inf.light_colour = LIGHTRED;
        inf.short_text   = make_stringf(C_("status", "lost %s"), msg.c_str());
    ...
    case STATUS_DRACONIAN_BREATH:
    {
        ...
        inf.light_text = C_("status", "Breath");
        inf.db_key     = "Breath";
    ...
    case STATUS_BLACK_TORCH:
        ...
        if (!yred_torch_is_raised())
        {
            if (yred_cannot_light_torch_reason().empty())
            {
                ...
                inf.light_text = C_("status", "Torch");
                inf.db_key     = "Torch";
            }
        }
        else
        {
            ...
            {
                inf.light_text = make_stringf(C_("status", "Torch (%d)"),
            ...
            inf.db_key     = "Torch";
            inf.short_text = C_("status", "lit torch");
        }
    break;

    case DUR_DIVINE_SHIELD:
    {
        inf.light_text = make_stringf(C_("status", "Shield (%d)"),
    ...
    case DUR_CORROSION:
        inf.light_text = make_stringf(C_("status", "Corr (%d)"),
                          (-1 * you.corrosion_amount()));
        inf.db_key     = "Corr";
        inf.short_text = make_stringf(C_("status", "corroded (%d)"), (-1 * you.corrosion_amount()));
        break;

    case DUR_FLAYED:
        inf.light_text = make_stringf(C_("status", "Flay (%d)"),
    ...
    case DUR_BERSERK:
        ...
        {
            inf.light_text = C_("status", "Bearserk");
            inf.db_key     = "Bearserk";
        }
        break;
    ...
    case STATUS_NO_POTIONS:
                ...
                && you.props.exists(RENOUNCE_POTIONS_TIMER_KEY)))
        {
            ...
            inf.light_text   = C_("status", "-Potion");
            inf.db_key       = "-Potion";
            inf.short_text   = C_("status", "unable to drink");
    ...
    case DUR_SWIFTNESS:
        ...
        {
            inf.light_text   = C_("status", "-Swift");
            inf.db_key       = "-Swift";
            inf.light_colour = RED;
            inf.short_text   = C_("status", "unswift");
    ...
    case STATUS_BEHELD:
        ...
        {
            inf.light_colour = RED;
            inf.light_text   = C_("status", "Mesm");
            inf.db_key       = "Mesm";
            inf.short_text   = C_("status", "mesmerised");
    ...
    case STATUS_PEEKING:
            ...
            && you.elapsed_time > 0)
        {
            ...
            inf.light_text   = C_("status", "Peek");
            inf.db_key       = "Peek";
            inf.short_text   = C_("status", "peeking");
    ...
    case STATUS_BACKLIT:
        if (you.backlit())
        {
            inf.short_text = C_("status", "glowing");
    ...
    case STATUS_UMBRA:
        if (you.umbra())
        {
            inf.short_text   = C_("status", "wreathed by umbra");
    ...
    case STATUS_NET:
        ...
        {
            inf.light_colour = RED;
            inf.light_text   = C_("status", "Held");
            inf.db_key       = "Held";
            inf.short_text   = C_("status", "held");
    ...
    case STATUS_LIQUEFIED:
    {
        if (you.liquefied_ground() || you.duration[DUR_LIQUEFYING])
        {
            ...
            inf.light_text   = C_("status", "SlowM");
            inf.db_key       = "SlowM";
            inf.short_text   = C_("status", "slowed movement");
    ...
    case DUR_POWERED_BY_DEATH:
    {
        ...
        if (pbd_str > 0)
        {
            ...
            inf.light_text   = make_stringf(C_("status", "Regen (%d)"), pbd_str);
            inf.db_key       = "Regen";
        }
        break;
    }
    ...
    case DUR_RAMPAGE_HEAL:
    {
        ...
        if (rh_pwr > 0)
        {
            ...
            inf.light_text   = make_stringf(rh_lvl < 2 ? C_("status", "MPRegen (%d)")
                                                       : C_("status", "Regen (%d)"), rh_pwr);
    ...
    case STATUS_MANUAL:
    {
        ...
        if (!skills.empty())
        {
            inf.short_text = C_("status", "studying ") + manual_skill_names(true);
    ...
    case STATUS_CONSTRICTED:
        if (you.is_constricted())
        {
            ...
            inf.light_text   = C_("status", "Constr");
            inf.db_key       = "Constr";

            if (you.constricted_type == CONSTRICT_ROOTS)
                inf.short_text   = C_("status", "constricted (roots)");
            else if (you.constricted_type == CONSTRICT_BVC)
                inf.short_text   = C_("status", "constricted (zombie hands)");
            else
                inf.short_text   = C_("status", "constricted");
        }
        break;
    ...
    case STATUS_SILENCE:
        if (silenced(you.pos()) && !you.duration[DUR_SILENCE])
        {
            ...
            {
                inf.light_colour = LIGHTRED;
                inf.light_text   = C_("status", "Sil");
                inf.db_key       = "Sil";
            }
            inf.short_text   = C_("status", "silenced");
            inf.long_text    = C_("status", "You are silenced.");
        }
    ...
    case STATUS_SERPENTS_LASH:
        if (you.attribute[ATTR_SERPENTS_LASH] > 0)
        {
            inf.light_colour = WHITE;
            inf.light_text
               = make_stringf(C_("status", "Lash (%u)"),
                              you.attribute[ATTR_SERPENTS_LASH]);
            inf.short_text = C_("status", "serpent's lash");
            inf.long_text = C_("status", "You are moving at supernatural speed.");
        }
        break;
    ...
    case STATUS_HEAVENLY_STORM:
        if (you.props.exists(WU_JIAN_HEAVENLY_STORM_KEY))
        {
            inf.light_colour = WHITE;
            inf.light_text
                = make_stringf(C_("status", "Heavenly (%d)"),
    ...
    case DUR_FUGUE:
    {
        ...
        inf.light_text = make_stringf(C_("status", "Fugue (%s%u%s)"),
    ...
    case DUR_WEREFURY:
        inf.light_text = make_stringf(C_("status", "Slay +%d"), you.props[WEREFURY_KEY].get_int());
    break;
    ...
    case STATUS_CLAUSTROPHOBIA:
        if (you.has_bane(BANE_CLAUSTROPHOBIA))
        {
            ...
            {
                inf.light_colour = LIGHTRED;
                inf.light_text = make_stringf(C_("status", "Phobia (-%d)"), stacks);
            }
        }
    break;
    ...
    case STATUS_BEOGH:
        ...
        {
            inf.light_colour = WHITE;
            inf.light_text = C_("status", "Beogh");
            inf.db_key     = "Beogh";
        }
        break;
    ...
    case DUR_FLOODED:
        ...
        inf.db_key      = "Flooded";
        inf.short_text  = C_("status", "flooded lungs");
    ...
    case STATUS_BRIBE:
    {
        ...
        if (bribe > 0)
        {
            ...
            inf.light_text = C_("status", "Bribe");
            inf.db_key     = "Bribe";
            inf.short_text = make_stringf(C_("status", "bribing [%s]"),
    ...
    case DUR_HORROR:
    {
        ...
        inf.light_text = make_stringf(C_("status", "Horr (%d)"), -1 * horror);
        inf.db_key     = "Horr";
        if (horror >= HORROR_LVL_OVERWHELMING)
        {
            inf.light_colour = RED;
            inf.short_text   = C_("status", "overwhelmed with horror");
            inf.long_text    = C_("status", "Horror overwhelms you!");
        }
        else if (horror >= HORROR_LVL_EXTREME)
        {
            inf.light_colour = LIGHTRED;
            inf.short_text   = C_("status", "extremely horrified");
            inf.long_text    = C_("status", "You are extremely horrified!");
        }
        else if (horror)
        {
            inf.light_colour = YELLOW;
            inf.short_text   = C_("status", "horrified");
            inf.long_text    = C_("status", "You are horrified!");
        }
        break;
    }
    ...
    case STATUS_CLOUD:
    {
        ...
        {
            inf.light_text = C_("status", "Cloud");
            inf.db_key     = "Cloud";
    ...
    case STATUS_STILL_WINDS:
        ...
        {
            inf.light_colour = BROWN;
            inf.light_text = C_("status", "-Clouds");
            inf.db_key     = "-Clouds";
        }
        break;
    ...
    case STATUS_DUEL:
        ...
        {
            inf.light_colour = WHITE;
            inf.light_text   = C_("status", "Duel");
            inf.db_key       = "Duel";
            inf.short_text   = C_("status", "duelling");
            inf.long_text    = C_("status", "You are engaged in single combat.");
        }
        break;
    ...
    case STATUS_CANINE_FAMILIAR_ACTIVE:
        ...
        {
            inf.light_colour = WHITE;
            inf.light_text   = C_("status", "Dog");
            inf.db_key       = "Dog";
            inf.short_text   = C_("status", "inugami summoned");
            inf.long_text    = C_("status", "Your inugami has been summoned.");
        }
        break;
    ...
    case STATUS_NO_SCROLL:
        ...
        {
            inf.light_colour = RED;
            inf.light_text   = C_("status", "-Scroll");
            inf.db_key       = "-Scroll";
            inf.short_text   = C_("status", "unable to read");
            inf.long_text    = C_("status", "You cannot read scrolls.");
        }
        break;
    ...
    case STATUS_RF_ZERO:
        ...
        inf.light_colour = RED;
        inf.light_text   = C_("status", "rF0");
        inf.db_key       = "rF0";
        inf.short_text   = C_("status", "fire susceptible");
        inf.long_text    = C_("status", "You cannot resist fire.");
        break;
    ...
    case STATUS_GRAVE_CLAW_UNAVAILABLE:
        ...
        {
            inf.light_colour = DARKGREY;
            inf.light_text = C_("status", "-GClaw");
            inf.db_key     = "-GClaw";
        }
        break;
    ...
    {
        inf.light_text = C_("status", "Destr");
        inf.db_key     = "Destr";
    ...
    case STATUS_CRUCIBLE_DEBT:
    {
        if (player_in_branch(BRANCH_CRUCIBLE))
        {
            ...
            {
                inf.light_text = C_("status", "Escape!");
                inf.db_key     = "Escape!";
                inf.light_colour = WHITE;
            }
        }
    ...
    case DUR_PARAGON_ACTIVE:
    {
        ...
        {
            inf.light_colour = WHITE;
            inf.light_text = C_("status", "Protected");
    ...
    case DUR_FORTRESS_BLAST_TIMER:
        inf.light_colour = WHITE;
        inf.light_text = C_("status", "Blast") + string(max(0, (40 - you.duration[DUR_FORTRESS_BLAST_TIMER]) / 10), '.');
        inf.db_key     = "Blast";
        inf.short_text = C_("status", "fortress blast");
    ...
    case DUR_TELEPORT:
        ...
        {
            inf.light_text   = C_("status", "!Tele!");
            inf.db_key       = "!Tele!";
            inf.light_colour = RED;
            inf.short_text   = C_("status", "teleporting to hostiles");
            inf.long_text    = C_("status", "You are about to teleport to other enemies.");
        }
        break;
    ...
    case STATUS_TRICKSTER:
        if (you.has_mutation(MUT_TRICKSTER))
        {
            ...
            if (bonus > 0)
            {
                inf.short_text = make_stringf(C_("status", "trickster (+%d AC)"), bonus);
    ...
    case STATUS_MNEMOPHAGE:
        if (!you.duration[DUR_ENKINDLED] && you.has_mutation(MUT_MNEMOPHAGE))
        {
            ...
            inf.light_text = make_stringf(C_("status", "Memories (%d)"), you.props[ENKINDLE_CHARGES_KEY].get_int());
            inf.db_key     = "Memories";
        }
        break;

    case DUR_ENKINDLED:
        inf.light_text = make_stringf(C_("status", "Enkindled (%d)"), you.props[ENKINDLE_CHARGES_KEY].get_int());
    ...
    case STATUS_SHROUD:
                ...
                && !you.duration[DUR_SHROUD_TIMEOUT])
        {
            ...
            inf.light_text   = C_("status", "Shroud");
            inf.db_key       = "Shroud";
            inf.short_text   = C_("status", "slimy shroud");
        }
        break;
    ...
    case STATUS_OSTRACISM:
        ...
        {
            inf.light_text = C_("status", "Ostracised");
    ...
    case STATUS_TESSERACT:
            ...
            && you.props.exists(TESSERACT_SPAWN_COUNTER_KEY))
        {
            ...
            inf.light_text = C_("status", "Tesseract");
    ...
    case STATUS_SUNDER_READY:
        ...
        {
            inf.light_colour = WHITE;
            inf.light_text = C_("status", "Sunder");
            inf.db_key     = "Sunder";
        }
        break;
    ...
    default:
        ...
        {
            inf.light_colour = RED;
            inf.light_text   = C_("status", "Missing");
            inf.db_key       = "Missing";
            inf.short_text   = C_("status", "missing status");
            inf.long_text    = C_("status", "Missing status description.");
            return false;
        }
        else
            break;
...
static void _describe_gem(status_info& inf)
{
    ...
    if (!gem_clock_active() && !you.gems_found[gem])
    {
        ...
        inf.light_text = C_("status", " Gem (*)");
        inf.db_key     = "Gem";
        inf.light_colour = CYAN;
        return;
    }
    const int d_aut_left = (limit - time_taken + 9) / 10;
    inf.light_text = make_stringf(C_("status", "Gem (%d)"), d_aut_left);
    inf.db_key     = "Gem";
...
static void _describe_zot(status_info& inf)
{
    ...
    if (lvl > 0)
    {
        inf.short_text = C_("status", "bezotted");
    ...
    inf.light_text = make_stringf(C_("status", "Zot (%d)"), turns_until_zot());
    inf.db_key     = "Zot";
    switch (lvl)
    {
        case 0:
...
static void _describe_glow(status_info& inf)
{
    ...
    if (you.magic_contamination >= 2000)
        ...
        inf.light_colour = YELLOW;
    inf.light_text = C_("status", "Contam");
    inf.db_key     = "Contam";
...
static void _describe_rev(status_info& inf)
{
    ...
    switch (tier)
    {
        case 1:
            ...
            inf.db_key       = "Rev";
            inf.short_text   = C_("status", "revving");
            inf.long_text    = C_("status", "You're starting to limber up.");
            return;
        ...
        case 2:
            ...
            inf.db_key       = "Rev";
            inf.short_text   = C_("status", "revving");
            inf.long_text    = C_("status", "You're limbering up.");
            return;
        ...
        case 3:
            ...
            inf.light_text   = C_("status", "Rev*");
            inf.db_key       = "Rev";
            inf.short_text   = C_("status", "revved");
            inf.long_text    = C_("status", "You're fully limbered up.");
            return;
    }
}
...
static void _describe_regen(status_info& inf)
{
    if (you.duration[DUR_TROGS_HAND])
    {
        ...
        inf.light_text = C_("status", "Regen Will++");
        inf.db_key     = "Regen Will++";
        inf.short_text = C_("status", "regenerating");
        inf.long_text  = C_("status", "You are regenerating.");
        _mark_expiring(inf, dur_expiring(DUR_TROGS_HAND));
    }
    else if (regeneration_is_inhibited())
    {
        inf.light_colour = RED;
        inf.light_text = C_("status", "-Regen");
        inf.db_key     = "-Regen";
        inf.short_text = C_("status", "inhibited regen");
        inf.long_text = C_("status", "Your regeneration is inhibited by nearby monsters.");
    }
}
...
static void _describe_poison(status_info& inf)
{
    ...
    inf.light_text   = C_("status", "Pois");
    inf.db_key       = "Pois";
    const bool zh = Options.language == lang_t::ZH;
    const string adj =
         (pois_perc >= 100) ? (C_("status", "lethally")) :
         (pois_perc > 65)   ? (C_("status", "seriously")) :
         (pois_perc > 35)   ? (C_("status", "quite"))
                            : (C_("status", "mildly"));
    inf.short_text   = zh ? adj + "中毒" : adj + C_("status", " poisoned");
...
static void _describe_speed(status_info& inf)
{
    ...
    {
        inf.light_colour = MAGENTA;
        inf.light_text   = C_("status", "Fast+Slow");
        inf.short_text   = C_("status", "hasted and slowed");
        inf.long_text = C_("status", "You are under both slowing and hasting effects.");
    }
    else if (slow)
    {
        inf.light_colour = RED;
        ...
        inf.db_key       = "Slow";
        inf.short_text   = C_("status", "slowed");
        inf.long_text    = C_("status", "You are slowed.");
    }
    else if (fast)
    {
        inf.light_colour = _dur_colour(BLUE, dur_expiring(DUR_HASTE));
        inf.light_text   = C_("status", "Fast");
        inf.db_key       = "Fast";
        inf.short_text = C_("status", "hasted");
        inf.long_text = C_("status", "Your actions are hasted.");
...
static void _describe_airborne(status_info& inf)
{
    ...
    inf.light_text   = C_("status", "Fly");
    inf.db_key       = "Fly";
    inf.short_text   = C_("status", "flying");
    inf.long_text    = C_("status", "You are flying.");
...
static void _describe_transform(status_info& inf)
{
    ...
    inf.light_text = C_("status", form->short_name.c_str());
    inf.short_text = C_("status", form->get_long_name().c_str());
...
static void _describe_invisible(status_info& inf)
{
    ...
    inf.light_text   = C_("status", "Invis");
    inf.db_key       = "Invis";
    inf.short_text   = C_("status", "invisible");
    if (you.backlit())
    {
        inf.light_colour = DARKGREY;
        inf.short_text += C_("status", " (but backlit and visible)");
    }
    inf.long_text = T_("You are ") + inf.short_text + T_(".");
...
static void _describe_channelled_spell(status_info& inf)
{
    ...
    switch (spell)
    {
        ...
        case SPELL_FLAME_WAVE:
            inf.light_colour = WHITE;
            inf.light_text   = C_("status", "Wave") + string(max(turns - 1, 0), '+');
            inf.db_key       = "Wave";
            break;

        case SPELL_SEARING_RAY:
            inf.light_colour = WHITE;
            inf.light_text   = C_("status", "Ray") + string(max(turns - 1, 0), '+');
            inf.db_key       = "Ray";
            break;
        ...
        case SPELL_CLOCKWORK_BEE:
            inf.light_colour = CYAN;
            inf.light_text = C_("status", "Winding") + string(max(turns - 1, 0), '.');
            inf.db_key     = "Winding";
            break;
