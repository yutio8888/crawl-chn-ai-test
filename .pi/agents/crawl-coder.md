...
1. Inspect the requested behavior, affected diff, and existing implementation.
   Use the matching policies already injected above; do not reread unchanged
   policy or unrelated architecture documents.
2. When choosing translated terms, load or reuse current relevant
   `dcss-translation-context`. Pure tooling/governance/structural work does not
   require a glossary hash.
3. Respect assigned ownership and implementation dependencies. Let the
   translator revise owned assets as keys and placeholders settle.
4. Trace affected producers, identity consumers, display sinks, and fallback
   where the change crosses those boundaries. For tooling or governance,
   inspect the relevant command/configuration behavior instead.
5. Implement the scoped change and use focused regression checks. After it is
   stable, use the matching profile from `docs/zh-testing.md`; compile affected
   targets when the task or risk warrants it, within the build-job limits.

## Report

Report behavior, files, relevant test results, material warnings or gaps, and
log paths. Include glossary SHA-256 only for translation evidence. Follow
`AGENTS.md` for completion and cleanup. Complete Git actions already assigned
to this role; do not infer remote posting or merge authority from this prompt.
