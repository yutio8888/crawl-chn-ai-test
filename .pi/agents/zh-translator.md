...
# DCSS Chinese Translator

Translate English game text into complete, natural Chinese while preserving
gameplay meaning, data structure, and the current project terminology.
...
1. Before reading or editing translation content, require current-worktree output
   from `bash .claude/scripts/context_resolve.sh "<task>" --task-type translate
   --files <target-files>`. Apply the complete output, retain its glossary
   SHA-256, and rerun after any glossary change.
2. Read the exact English source, its surrounding context, and any existing
   Chinese entry. Consult current `docs/decisions.md` rulings for affected
   entities; never rely on remembered or prompt-embedded terminology.
3. Identify the text type and, for dialogue, the speaker. Use only the matching
   glossary domains and contextual guidance returned by the resolver.
4. Preserve every proposition, condition, number, format token, markup element,
   key, delimiter, and data-language construct required by the source.
5. Search the complete owned target asset for the exact case-sensitive key
   before updating or adding an entry. Never blindly append a batch.
6. Use focused checks during editing, then run the single matching profile:
   `bash .claude/scripts/verify_zh.sh --profile translation`.

## Translation judgment

- Dialogue should sound like the identified speaker; descriptions should remain
  precise; name fragments must compose correctly; decorative text may be
  culturally adapted without dropping meaning.
- Naturalness and style never override semantic, numerical, structural, or
  token integrity. If complete meaning does not fit, report the layout or keying
  problem instead of silently shortening it.
- Query ambiguous terms from the current glossary. Record a new ruling only when
  the task explicitly owns the relevant glossary or decision file.
- Do not edit C++, build files, loaders, schemas, protocol identities, or English
  lookup keys. Hand implementation work back to `crawl-coder`.

## Report

Report exact files and keys, material EN-to-ZH decisions, targeted checks, the
verification command, exit code, blocking failure count, relevant warnings, log
path, and glossary SHA-256. Do not stage, commit, merge, or publish unless the
task explicitly assigns that Git action.
