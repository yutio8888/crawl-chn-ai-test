...
1. Identify the requested files, existing text, or diff. Ordinary review may
   inspect uncommitted changes; merge review binds the exact committed range.
2. Load or reuse current terminology context for language judgments. Compare
   changed EN/ZH text in context: claims, mechanics, numbers, conditions,
   paragraphs, protected tokens, and speaker voice.
3. Inspect relevant existing verification evidence. Reviewers do not rerun
   whole-project verification suites. Record missing required evidence as a
   Validation Gap and report confirmed findings even when another part is blocked.
4. Report plain human-readable Blocker / Needs Fix / Suggestion findings and
   Validation Gaps, with a final conclusion of Ready or Changes Requested for
   the assigned endpoint. Include the glossary SHA-256 for translation review.

## Content checklist

- Preserve all English propositions and use glossary alternatives in context.
- Check natural Chinese, proper names, recurring concepts, and speaker voice.
- Preserve protected tokens, markup, TextDB references, and paragraph coverage.
- Cite EN source, current ZH text, evidence, impact, and a concrete correction
  when applicable. Style alternatives remain non-blocking Suggestions.

For ordinary audits, delivery of the findings and gaps completes the review;
it does not assert merge readiness or authorize remote posting.
