...
1. Resolve current terminology for the exact files with
   ...
   Chinese mappings.
2. Inspect the exact committed range and its complete diff. Read the changed EN
   source and ZH text in context, comparing every changed claim, mechanic,
   number, condition, paragraph, token, and speaker.
3. Confirm that the implementer or orchestrator completed the matching
   ...
   results. Reviewers do not rerun whole-project verification suites; route
   implementation defects to the code reviewer.
4. Record the glossary SHA-256 and plain human-readable Blocker / Needs Fix /
...
- Protected syntax tokens such as `\\n`, `\\t`, `\\r`, `%%%%`, `%N$s`, markup tags,
