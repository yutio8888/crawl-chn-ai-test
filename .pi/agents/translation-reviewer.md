...
expensive independent suites. The final gate owns the required `run_all.sh`
tooling suite and runs it once after all reviewers report Ready, against the
exact candidate OID. If the task or release contract requires additional suites
such as `help-full` or `post_zh_runtime.sh full`, run each once only after all
reviewers report Ready, against the same final candidate. A reviewer-requested
fix creates a new bundle and uses targeted checks during the next feedback loop;
do not spend full-suite evidence on a candidate already known to need changes.
...
The review profile also always runs the required `tooling-tests` phase after
`review-ledgers`, reading `.claude/scripts/tests/run_all.sh` from the exact
candidate commit's Git blob and executing it with the candidate worktree path
as `$0`, so it discovers the candidate test files. It fixes
`PYTHONSAFEPATH=1` and `ZH_TOOLING_TEST_JOBS=2`. The corresponding worktree path
must be a regular, non-symlink file; a missing or unsafe path, an unreadable Git
blob, and any nonzero suite exit fail closed. This is the single final-gate
tooling-suite run for that reviewed candidate.
...
strict review ledgers, tooling tests, the final approval decision, or the
read-only merge gate: `review-ledgers` and `tooling-tests` always run locally,
every routed reviewer must already be Ready, and the approval is still sealed
from the same immutable attempt digest. The trusted contract section in
...
When the caller supplies both `--github-actions-run` and the explicit
`--github-actions-fallback-local` opt-in, the gate may fall back to its existing
fully local phase plan only if the trusted proof helper classifies GitHub API,
network, authentication, or `gh` availability failure with its dedicated
unavailable result. The verifier is then started once on the exact candidate;
it does not rerun review ledgers or tooling tests, and verification metadata
records `source=local-fallback`, the stable unavailable reason, and the
requested run id. A remote run or required job that reports failure,
cancellation, or incompleteness; any repository, commit, workflow, event, or
job-inventory mismatch; malformed or tampered proof data; and non-availability
HTTP errors or unclassified `gh` command failures never fall back. Without the
explicit opt-in, unavailable GitHub proof remains a blocking error.
Historical frozen contracts that predate the fallback field remain valid
read-only evidence: an absent `allow_local_fallback` is interpreted as false,
never added to or migrated into the sealed evidence.
