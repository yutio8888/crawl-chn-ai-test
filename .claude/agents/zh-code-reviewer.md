...
- Cross-reference with rulings in DECISIONS.md:
...
- These CANNOT be confused. If `法力` describes spell strength, it's an error.
...
- Evocations = 激活技能 (NOT 召唤术, which is Summoning)

**Check 19 — Entry count drift:**
- Run: `check_consistency.sh --stale`
...
- Cross-reference with `character_voice_guide_v2.md`:
...
**AP-1 — Omissions in translated files:** File has ZH guards but English messages remain in the same function/switch block.
→ Check: grep `lang_t::ZH` count vs grep English strings in the same file.

**AP-2 — Layer 3 return values:** `const char*` functions with switch/case returning hardcoded English, no ZH branch.
→ Check: `scan_untranslated.sh --layer3`
