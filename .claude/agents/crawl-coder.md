...
1. **编译检查**：`make -j4 2>&1 | grep -E '^(.*error:|.*warning:)'` — 零错误才继续（C++ 模板错误可能跨多行，`tail` 会截断上下文）
2. **格式检查**（若涉及 TextDB）：
   - `grep -c '^%%%%$' <zh_file>` → 与 EN 文件对比 %%%% 数量
   - `grep -oP '@[a-zA-Z_][a-zA-Z_0-9 ]*@' <zh_file> | sort -u` → 检查 @keyword@ 引用完整性
