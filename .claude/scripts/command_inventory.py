...
def _default_inventory_output() -> str:
    ...
    return os.path.join(root, "command-inventory.json")
    ...
    * Directories are never auto-created and no component is followed as
    ...
    if not p.is_absolute():
        print(
            ...
            f"under the configured temp root; got {raw!r} (relative path "
            f"rejected)",
            file=sys.stderr)
        raise SystemExit(1)
    ...
    if raw_prefix not in allowed_prefixes:
        print(
            ...
            f"an allowed temporary root {allowed_prefixes!r}; refusing to "
            f"write",
            file=sys.stderr)
        raise SystemExit(1)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="output JSON path; must be an absolute path that is exactly "
             ...
             "root (the canonical root-owned sticky temp root; realpath /"
             "private/tmp on macOS when the default root is used) -- "
             "nested components, '.', '..', relative paths, "
