...
# Issue-16 monspeak VISUAL channel contract (CR-004/CR-008): the complete
# sorted-unique (canonical key, variant ordinal) identity set of the EN
# monspeak variants that open the VISUAL: channel at the baseline OID
# (b3ad4425053c2175284d32441d67218df97035b0).  The contract is fail-closed
# against EN drift: a removed/reworded EN VISUAL line or a moved position
# (even one jointly mirrored in ZH so the per-position channel check still
# passes) changes this set and trips the freeze before the per-position ZH
# check runs.  The count is derived from the set, never a separate constant.
MONSPEAK_EN_VISUAL_POSITIONS = (
    ("'r'", 0),
    ('_agnes_common_', 0),
    ('_aizul_common_', 0),
    ('_aizul_common_', 3),
...
MONSPEAK_EN_VISUAL_POSITION_COUNT = len(MONSPEAK_EN_VISUAL_POSITIONS)
...
def _monspeak_visual_channel_findings(source_dir):
    """Issue-16 monspeak VISUAL channel routing (CR-004/CR-008).

    The complete sorted-unique EN (canonical key, variant ordinal) set of
    VISUAL positions is frozen from the baseline dump and compared exactly,
    so an EN edit that swaps two positions (even when jointly mirrored in
    ZH, keeping the total at 496 and the per-position ZH check satisfied)
    still fails.  For every frozen EN position the ZH variant at the same
    key and ordinal must also start with ``VISUAL:``: the candidate ZH is
    aligned per-key to EN, so this pins the channel routing without binding
    any Chinese sentence text."""
    ...
    visual_positions = sorted(
        ...
        if en[key][ordinal].startswith("VISUAL:")
    )
    if visual_positions != list(MONSPEAK_EN_VISUAL_POSITIONS):
        frozen = set(MONSPEAK_EN_VISUAL_POSITIONS)
        current = set(visual_positions)
        missing = sorted(frozen - current)
        extra = sorted(current - frozen)
        findings.append((
            contract_id, 'dat/database/monspeak.txt',
            f"EN VISUAL position set drifted from the frozen baseline "
            f"identity set: {len(visual_positions)} positions "
